...
public class DBCart {
    ...
    public void displayOrderInfo()
    {
        ...
        System.out.println("-----------------------------------------------------------------------------");
        ArrayList<Cart> list = getUserCart("15563866750");
        if(list.size() == 0){
    ...
    public boolean insertNewCart(String user, String mealSerialNumber, int qty)
    {
        boolean result =false;
        try {
            String sqlInset = "insert into cart(cartSN,userTel,mealSerialNumber,qty,mealName,mealPrice) values(?, ?, ?, ?, ?, ?)";
            ...
            m = menu.getMeal(mealSerialNumber);
    ...
    public double getPriceInCart(String cartSN)
    {
        double totalPrice = 0;
        Cart cart = getCartBySN(cartSN);
        ArrayList<OrderItem> itemList =  cart.getOrderlist();
